...
const getBookingType = (booking: Booking) => {
    if (booking.houseboatId) return { icon: Ship, path: `/dashboard/houseboat-reservations?highlight=${booking.id}` };
    if (booking.restaurantTableId) return { icon: Utensils, path: '/dashboard/restaurant-reservations' };
    if (booking.dailyTravelPackageId) return { icon: Calendar, path: '/dashboard/daily-travel-reservations' };
...
export default function DashboardNav() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(notificationsQuery, 
      (snapshot) => {
        const fetchedNotifications = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Booking))
            .filter(booking => booking.status === 'Pending'); // Filter on the client
  ...
  const handleDismissNotification = async (e: React.MouseEvent, notificationId: string) => {
    ...
    e.preventDefault();
    if (!firestore) return;
    const notifRef = doc(firestore, 'bookings', notificationId);
  ...
  const handleNotificationClick = async (notification: Booking) => {
    ...
    const notifRef = doc(firestore, 'bookings', notification.id);
    await setDoc(notifRef, { read: true }, { merge: true });
  ...
  const handleMarkAllRead = async () => {
    ...
    notifications.forEach(notif => {
        if (!notif.read) {
            const notifRef = doc(firestore, 'bookings', notif.id);
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications && notifications.length > 0 ? (
                    notifications.map(notif => {
                        ...
                        const isExpired = differenceInHours(new Date(), new Date(notif.startTime)) > 48;
                        return (
                          <DropdownMenuItem key={notif.id} onSelect={() => handleNotificationClick(notif)} className={cn("flex gap-3 pr-8", !notif.read && 'bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:hover:bg-blue-900/50')}>
                              ...
                              <div className="flex-grow">
                                <div className="flex items-center gap-2">
                                  <p className="text-sm font-medium">{notif.clientName}</p>
                                  {isExpired && <Badge variant="destructive" className="px-1 py-0 text-[10px]">Expired</Badge>}
                                </div>
                                <p className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(notif.startTime), { addSuffix: true })}</p>
                              </div>
                               <Button
                                variant="ghost"
                                size="icon"
