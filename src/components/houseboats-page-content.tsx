...
import type { HouseboatModel, HouseboatModelPrice, Tariff } from '@/lib/data-firestore';
import {
  Select,
  SelectContent,
  SelectItem,
...
const HouseboatsPageInner = ({ dictionary, initialData }: HouseboatsPageContentProps) => {
  ...
  const filteredAndSortedBoats = useMemo(() => {
    ...
    if (guestFilter !== 'all') {
      ...
      boats = boats.filter(boat => boat.maximumCapacity >= minGuests);
    }

    // Sort
    boats.sort((a, b) => {
      // Push unavailable boats to the bottom
      if (isSearchView) {
