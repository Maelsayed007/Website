...
export default function PackageCard({ pkg, index, dateRange, guests }: PackageCardProps) {
    ...
    return (
        <div className="relative mt-8 bg-white rounded-3xl border-2 border-[#18230F] p-4 pt-6 md:p-6 shadow-none">
            {/* Cut-out Labels on Border */}
            ...
            {/* Boats Grid - Using Full Cards */}
            <div className="grid grid-cols-1 gap-4 mt-2">
                {pkg.boats.map((boat, i) => (
                    <HouseboatSearchCard
                        key={`${pkg.id}-boat-${i}`}
                        boat={boat}
                        // Disable hint inside package by passing 0
                        requestedGuests={0}
                        hidePrice={true}
                    />
                ))}
