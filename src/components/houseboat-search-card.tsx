...
import { Users, Bed, Anchor, Star, Heart, Utensils, Droplets, Bath, Sofa, MapPin } from 'lucide-react';
...
export default function HouseboatSearchCard({ boat, requestedGuests, hidePrice = false }: HouseboatSearchCardProps) {
  const searchParams = useSearchParams();
  const images = boat.imageUrls || (boat as any).images || [];
  ...
  const priceDisplay = boat.totalPrice
    ...
    : `€${boat.pricePerNight || (boat as any).startingPrice || 150}`;
      ...
      <div className="flex-1 p-3 md:p-4 flex flex-col relative">
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-auto py-2.5 border-t border-gray-50">
          ...
          <div className="flex items-center gap-2 text-sm md:text-base text-gray-600">
            ...
            <span className="font-semibold whitespace-nowrap">{boat.bedrooms} Cabins</span>
          </div>
          <div className="flex items-center gap-2 text-sm md:text-base text-gray-600">
            <div className="w-7 h-7 rounded-full bg-emerald-50 flex items-center justify-center">
        ...
        <div className="mt-3 flex items-center justify-end gap-3">
          {(requestedGuests || 0) > 0 && boat.optimalCapacity && requestedGuests! > boat.optimalCapacity && (
            <span className="bg-[#18230F] text-white rounded-full px-2 py-0.5 text-[10px] whitespace-nowrap font-medium">
              + {requestedGuests! - boat.optimalCapacity} Guests with extra bed in the living room
            </span>
          )}
